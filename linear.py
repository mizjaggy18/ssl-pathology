...
class Net(nn.Module):
    def __init__(self, opt):
        ...
        if (not opt.random) and (not opt.pretrained):
            ...
            # # load model trained from main.py
            ...
            state = torch.load(opt.model_path, map_location='cuda:0')
        ...
        self.fc = nn.Linear(opt.output_dims, 4, bias=True)
...
def train_val(net, data_loader, train_optimizer):
    ...
    # all_preds, all_labels, all_slides, all_outputs0, all_outputs1, all_outputs2, all_outputs3, all_patches  = [], [], [], [], [], [], [], []
    ...
    with (torch.enable_grad() if is_train else torch.no_grad()):
        for data, _, target, patch_id, slide_id in data_bar:
            ...
            # total_correct += torch.sum((prediction[:, 0:3] == target.unsqueeze(dim=-1)).any(dim=-1).float()).item()
